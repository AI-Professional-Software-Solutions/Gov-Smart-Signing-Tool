...
export const ThreeStateToggle = <T,>({
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (state === null) {
      ...
      toggleState(isRightHalf ? rightValue : leftValue)
      return
    }
    toggleState(state === leftValue ? rightValue : leftValue)
  }
    ...
    <div className="flex items-center justify-center space-x-5">
      <div className="flex items-center justify-end flex-1">
        <div className="cursor-pointer select-none relative" onClick={() => toggleState(leftValue)}>
          <span>{leftLabel}</span>
          <span className="ml-2 text-xs absolute right-0 top-8 w-64 text-right">{leftDescription}</span>
        </div>
      </div>
      ...
      >
        <span
          className={`absolute transition-transform ml-1 duration-300 transform ${
            state === rightValue ? 'translate-x-12' : state === leftValue ? 'translate-x-0' : 'translate-x-6'
      ...
      <div className="flex items-center justify-start flex-1">
        <div className="cursor-pointer select-none relative" onClick={() => toggleState(rightValue)}>
          <span>{rightLabel}</span>
          <span className="mr-2 text-xs absolute left-0 top-8 w-64 text-left">{rightDescription}</span>
        </div>
      </div>
    </div>
  )
