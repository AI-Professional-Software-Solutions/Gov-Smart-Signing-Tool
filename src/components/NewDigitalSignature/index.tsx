...
const translationsObject = {
  ...
  ro: {
    ...
    wellDone: "Felicitări! Semnătura ta digitală este gata.",
    wellDoneDescription: "Acum poți semna documente folosind noua ta semnătură digitală.",
    secondaryContent: {
      branch: {
...
export const NewDigitalSignature: React.FC<NewDigitalSignatureProps> = ({
  ...
  useEffect(() => {
    if (generationMethod === "brainwallet") {
      setValue(
        "passphrase",
        generateRandomWords(10, translationsObject[currentLanguage].words)
          .split(" ")
          .slice(0, 10)
          .join(" "),
      );
      ...
      <ResponsiveLayout
        primaryContent={
          <div ref={parentDivRef} className="w-full rounded z-10 relative overflow-visible">
            ...
            >
              <PublicKeyStep publicKey={publicKey} alias={watch("signatureAlias")} />
            </div>
            ...
            >
              <PrivateKeyStep
                ...
                onKeyDownloaded={() => setWasKeyDownloaded(true)}
              />
            </div>
            ...
            >
              <div className="text-center">
                <h2 className="text-2xl font-bold mb-4">
                  {translationsObject[currentLanguage].wellDone}
                </h2>
                <p className="text-lg mb-4">
                  {translationsObject[currentLanguage].wellDoneDescription}
                </p>
              </div>
            </div>
          </div>
            ...
            {step !== "confirmation" && (
              <button
                ...
                disabled={
                  ...
                  (step === "passphrase" &&
                    watch("passphrase").trim().split(/\s+/).length !== 10) ||
